...
def init_distributed_mode(args):
    ...
    torch.distributed.barrier(device_ids=[int(args.local_rank)])
    ...
    if is_dist_avail_and_initialized():
        ...
        if device == torch.device('cpu'):
            raise EnvironmentError(
                'No GPUs, cannot initialize multigpu training.')
        model.to(device)
        # optimizer = torch.optim.Adam(model.parameters(), lr=lr)
        # model, optimizer = amp.initialize(model, optimizer, opt_level="O0")
        ddp_sub_model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[gpu_num], find_unused_parameters=find_unused_parameters)
        model = ddp_sub_model
